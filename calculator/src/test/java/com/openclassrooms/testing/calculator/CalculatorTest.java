...
class CalculatorTest {
	...
	void multiply_shouldReturnTheProduct_ofTwoIntegers() {
		...
		/// Assert
		assertThat(462).isEqualTo(produit);
	}
	
	@ParameterizedTest(name = "{0} x 0 doit étre égal à 0")
	@ValueSource(ints = {1, 2, 42, 1011, 5089 })
	public void multiply_shouldReturnZero_ofZeroWithMultipleIntegers(int arg) {
		// Arrange -- Tout est prét !
		
		//Act --Multiplier par zero
		int actualResult = calculatorUnderTest.multiply(arg, 0);
		
		// Assert -- ça  vaut toujours zero !
		assertThat(actualResult).isEqualTo(0);
	}
	
	@ParameterizedTest(name = "{0} + {1} shoud equal to {2}")
